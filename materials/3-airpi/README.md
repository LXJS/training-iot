...
5. Deploy this application to the Raspberry Pi with `n2 reactor install 'Raspberry Pi' airpi`. This sends a message to the Raspberry Pi to ask it to install the npm module 'airpi' in a container called 'airpi' on the Raspberry Pi. It knows to install this on your Raspberry Pi because you only have one device named 'Raspberry Pi' that is visible to you.
6. We can monitor this installation with `n2 reactor state 'Raspberry Pi'`. This will display the current state of the reactor as it moves from 'installing' to 'stopped' once the application has finished installing in its instance.
...
9. Just watching telemetry is no fun by itself. Let's add this device to a global climate monitoring app!
